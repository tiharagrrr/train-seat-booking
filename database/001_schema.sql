...
CREATE TABLE coach_templates (
  ...
  rate_per_km_lkr numeric(8,2) NOT NULL CHECK(rate_per_km_lkr>=0), 
  display_order integer NOT NULL,
  UNIQUE(train_template_id,coach_code), 
  UNIQUE(train_template_id,display_order)
);
...
CREATE TABLE seat_templates (
  ...
  active boolean NOT NULL DEFAULT true, 
  UNIQUE(coach_template_id,seat_number), 
  UNIQUE(coach_template_id,row_number,column_number)
);

CREATE TABLE service_schedules (
  id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  route_id bigint NOT NULL REFERENCES routes(id), 
  train_template_id bigint NOT NULL REFERENCES train_templates(id),
...
CREATE TABLE service_runs (
  id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  schedule_id bigint REFERENCES service_schedules(id), 
  route_id bigint NOT NULL REFERENCES routes(id),
  service_code text NOT NULL, 
...
CREATE TABLE coaches (
  ...
  source_template_id bigint REFERENCES coach_templates(id), 
  coach_code text NOT NULL, 
  class_name text NOT NULL,
  reserved boolean NOT NULL DEFAULT true, 
  rate_per_km_lkr numeric(8,2) NOT NULL CHECK(rate_per_km_lkr>=0), 
...
CREATE TABLE seats (
  ...
  source_template_id bigint REFERENCES seat_templates(id), 
  seat_number text NOT NULL,
  ...
  UNIQUE(coach_id,row_number,column_number)
);
...
CREATE TABLE bookings (
  ...
  fare_lkr integer NOT NULL CHECK(fare_lkr>=0), 
  currency char(3) NOT NULL DEFAULT 'LKR',
...
CREATE TABLE waitlist_entries (
  ...
  segment int4range NOT NULL, 
  class_name text NOT NULL, 
  quoted_fare_lkr integer NOT NULL,
