...
SELECT r.id, s.id, x.position, x.km 
FROM routes r
JOIN (
  VALUES 
    ('FOT', 0, 27.0),
    ...
    ('ELL', 7, 32.0),
    ('BAD', 8, 0.0)
) x(code, position, km) ON true
JOIN stations s ON s.code = x.code 
WHERE r.name = 'Colombo Fort–Badulla';

INSERT INTO train_templates (name) 
VALUES ('Udarata Menike Standard Formation');

INSERT INTO coach_templates (train_template_id, coach_code, class_name, reserved, rate_per_km_lkr, display_order)
SELECT t.id, x.code, x.class_name, x.reserved, x.rate, x.ord 
FROM train_templates t 
CROSS JOIN (
  VALUES 
    ('R1', 'First Class', true, 14.00, 1),
    ('R2', 'Second Class', true, 9.00, 2),
    ('R3', 'Second Class', true, 9.00, 3),
    ('U1', 'Third Class', false, 4.00, 4),
    ('U2', 'Third Class', false, 4.00, 5),
    ('U3', 'Third Class', false, 4.00, 6),
    ('U4', 'Third Class', false, 4.00, 7),
    ('U5', 'Third Class', false, 4.00, 8)
) x(code, class_name, reserved, rate, ord);
...
SELECT 
  ...
  s.route_id, 
  s.service_code, 
  date_trunc('day', now()) + interval '1 day 05:55' 
...
JOIN service_schedules ss ON ss.id = sr.schedule_id 
JOIN coach_templates ct ON ct.train_template_id = ss.train_template_id;
