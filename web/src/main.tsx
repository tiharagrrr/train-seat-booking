...
function App() {
  ...
  useEffect(() => {
    fetch(`${API}/api/runs`)
      ...
      .then((x: Run[]) => {
        setRuns(x);
        if (x[0]) setRunId(x[0].id);
      });
  }, []);

  useEffect(() => {
    if (!runId) return;
    fetch(`${API}/api/runs/${runId}/stations`)
      .then((r) => r.json())
      .then((x: Station[]) => {
        setStations(x);
        setOriginId(x[0]?.id);
  ...
  async function loadSeats() {
    ...
    const r = await fetch(
      `${API}/api/availability?runId=${runId}&originId=${originId}&destinationId=${destinationId}`
    );
    const body = await r.json();

    setSeats(r.ok ? body : []);
    if (!r.ok) setMessage(body.message ?? 'Choose a valid journey.');
    setLoading(false);
  }

  useEffect(() => {
    if (destinationId) loadSeats();
  ...
  async function book(e: React.FormEvent) {
    e.preventDefault();
    if (!selected || !runId || !originId || !destinationId) return;
    ...
    if (r.ok) {
      ...
      setMessage(body.message ?? 'We could not complete this booking.');
      if (r.status === 409) loadSeats();
    }
  }
  ...
  return (
    <main>
      <header>
        ...
        <span className="tag">Sri Lanka Railways · Reserved travel</span>
      </header>
      ...
      <section className="hero">
        ...
        <p>
          Reserve a comfortable seat across the hill country. Fair fares,
          flexible segments, one unforgettable journey.
        </p>
      </section>
      ...
      <section className="search card" aria-label="Journey selection">
        ...
        <button onClick={loadSeats} disabled={loading}>
      ...
      {seats.length > 0 && (
        <section className="booking">
          <div className="map card">
            <div className="section-title">
              <div>
                ...
                <h2>
                  {origin?.name} <span>to</span> {destination?.name}
                </h2>
              </div>
              <div className="legend">
                <i />
                Available <i className="taken" />
                Taken
              </div>
            </div>
          ...
          <aside className="summary card">
            ...
            <h2>
              {origin?.code} <ArrowRight /> {destination?.code}
            </h2>
            ...
            {selected ? (
              <>
                ...
                <div className="fare">
                  ...
                  <strong>
                    LKR {(selected.fareCents / 100).toLocaleString()}
                  </strong>
                  <small>Calculated only for your selected distance</small>
                </div>
                ...
                <form onSubmit={book}>
                  ...
                  <button disabled={loading}>
                    {loading ? 'Securing seat…' : 'Confirm booking'}
                  </button>
                </form>
              </>
            ) : (
              <p className="empty">
                Choose an available seat to see your fare and continue.
              </p>
            )}

            {message && (
              <p className={reference ? 'success' : 'notice'}>
